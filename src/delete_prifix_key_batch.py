...
def delete_keys_by_prefix(redis_client, prefix, batch_size=1000, dry_run=True):
    """
    批量删除以指定前缀开头的Redis KEY
...
def main():
    ...
    if len(sys.argv) < 2:
        logger.error("请提供要删除的 KEY 前缀")
        print("使用方法: python delete_prifix_key_batch.py <前缀> [--execute]")
        print("示例: python delete_prifix_key_batch.py CWInfoIncompleteTag_")
        print("      python delete_prifix_key_batch.py CWInfoIncompleteTag_ --execute")
        sys.exit(1)
    ...
    if dry_run:
        ...
        response = input(f"确认要删除所有以 '{prefix}' 开头的 KEY 吗？(yes/no): ")
    ...
    try:
        total_count = delete_keys_by_prefix(redis_client, prefix, dry_run=dry_run)
